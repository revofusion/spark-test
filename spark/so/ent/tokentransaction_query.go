...
// QueryPeerSignatures chains the current query on the "peer_signatures" edge.
func (ttq *TokenTransactionQuery) QueryPeerSignatures() *TokenTransactionPeerSignatureQuery {
	query := (&TokenTransactionPeerSignatureClient{config: ttq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := ttq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (ttq *TokenTransactionQuery) QuerySparkInvoice() *SparkInvoiceQuery {
	query := (&SparkInvoiceClient{config: ttq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := ttq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithPeerSignatures tells the query-builder to eager-load the nodes that are connected to
// the "peer_signatures" edge. The optional arguments are used to configure the query builder of the edge.
func (ttq *TokenTransactionQuery) WithPeerSignatures(opts ...func(*TokenTransactionPeerSignatureQuery)) *TokenTransactionQuery {
	query := (&TokenTransactionPeerSignatureClient{config: ttq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	ttq.withPeerSignatures = query
	return ttq
}
...
func (ttq *TokenTransactionQuery) WithSparkInvoice(opts ...func(*SparkInvoiceQuery)) *TokenTransactionQuery {
	query := (&SparkInvoiceClient{config: ttq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	ttq.withSparkInvoice = query
	return ttq
}
...
func (ttq *TokenTransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*TokenTransaction, error) {
	var (
		...
		loadedTypes = [8]bool{
			...
			ttq.withPeerSignatures != nil,
			ttq.withSparkInvoice != nil,
		}
	)
	...
	if query := ttq.withPeerSignatures; query != nil {
		if err := ttq.loadPeerSignatures(ctx, query, nodes,
			func(n *TokenTransaction) { n.Edges.PeerSignatures = []*TokenTransactionPeerSignature{} },
			func(n *TokenTransaction, e *TokenTransactionPeerSignature) {
				n.Edges.PeerSignatures = append(n.Edges.PeerSignatures, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := ttq.withSparkInvoice; query != nil {
		if err := ttq.loadSparkInvoice(ctx, query, nodes,
			func(n *TokenTransaction) { n.Edges.SparkInvoice = []*SparkInvoice{} },
			func(n *TokenTransaction, e *SparkInvoice) { n.Edges.SparkInvoice = append(n.Edges.SparkInvoice, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (ttq *TokenTransactionQuery) loadPeerSignatures(ctx context.Context, query *TokenTransactionPeerSignatureQuery, nodes []*TokenTransaction, init func(*TokenTransaction), assign func(*TokenTransaction, *TokenTransactionPeerSignature)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*TokenTransaction)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.TokenTransactionPeerSignature(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tokentransaction.PeerSignaturesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.token_transaction_peer_signatures
		if fk == nil {
			return fmt.Errorf(`foreign-key "token_transaction_peer_signatures" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "token_transaction_peer_signatures" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (ttq *TokenTransactionQuery) loadSparkInvoice(ctx context.Context, query *SparkInvoiceQuery, nodes []*TokenTransaction, init func(*TokenTransaction), assign func(*TokenTransaction, *SparkInvoice)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*TokenTransaction)
	nids := make(map[uuid.UUID]map[*TokenTransaction]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tokentransaction.SparkInvoiceTable)
		s.Join(joinT).On(s.C(sparkinvoice.FieldID), joinT.C(tokentransaction.SparkInvoicePrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tokentransaction.SparkInvoicePrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tokentransaction.SparkInvoicePrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*SparkInvoice](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "spark_invoice" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
