...
func feeBumpAndConfirmTx(t *testing.T, client *rpcclient.Client, faucet *sparktesting.Faucet, tx *wire.MsgTx) {
	...
	require.NoError(t, err)
	feeBumpTx, err := sparktesting.SignFaucetCoinFeeBump(anchorOutPoint, coin, outputScript)
	require.NoError(t, err)
